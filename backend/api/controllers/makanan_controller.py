...
def get_by_id(id_makanan: str) -> Response:
    ...
    try:
        ...
        # Jalankan query untuk mengambil data makanan
        result: MakananModel = session.query(MakananModel).filter_by(id_makanan=id_makanan).first()
...
def update(id_makanan: str) -> Response:
    ...
    try:
        ...
        # Jalankan query untuk mengambil data makanan
        makanan: MakananModel = session.query(MakananModel).filter_by(id_makanan=id_makanan).first()

        # Jika makanan tidak ditemukan, tampilkan pesan error
        if not makanan:
            return jsonify({"error": "Makanan not found!"}), 404
