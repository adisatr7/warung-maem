...
def add() -> Response:
    try:
        ...
        pembelian = PembelianModel(
            data["id_transaksi"], data["id_makanan"], data["kuantitas"]
        )
...
def get_by_id(id_pembelian: str) -> Response:
    try:
        ...
        # Jalankan query untuk mengambil data pembelian
        pembelian = (
            session.query(PembelianModel)
            .filter(PembelianModel.id_pembelian == id_pembelian)
            .first()
        )
        ...
        if not pembelian:
            ...
            return jsonify({"error": "pembelian not found!", "code": 404})
